...
export default function TeamsClient({ teams }: { teams: Team[] }) {
  ...
  const toggleFav = (id: string) => {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("favTeams", JSON.stringify(next));
    }
  };

  const prefTeams = teams.filter(t => t.prefecture === pref);
  const nq = normalize(q);
  const filtered = useMemo(() => prefTeams.filter(t => {
    ...
    return true;
  }), [prefTeams, cat, nq, feeMax, practice, selectionOnly, jleagueOnly]);
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,260px) 1fr", gap: 20, alignItems: "start" }}>
        ...
        <section>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
            <div style={{ fontSize: 13, color: "var(--kf-muted)" }}>{`${filtered.length}件のチーム`}</div>
